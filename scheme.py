...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 10
        instance_frame = Frame(self)
        while formals is not nil:
            instance_frame.define(formals.first, vals.first)
            vals = vals.rest
...
def do_and_form(expressions, env):
    ...
    while expressions.rest is not nil:
        firstexp = scheme_eval(expressions.first, env)
        if is_false_primitive(firstexp):
            return firstexp
        if is_true_primitive(firstexp):
            expressions = expressions.rest
